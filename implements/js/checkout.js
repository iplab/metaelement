	...
	function HTTPlinks() {
		if(document.getElementById('nav')) {
			...
			var countryURL 	= {US:'http://www.bbcamericashop.com', CA:'http://www.bbccanadashop.com'};
			for (var i=0;i<elements.length;i++) {
			    var cLink = elements[i].getAttribute('href');
			    if(cLink!='#') {
			        if(cLink.indexOf('http')<0 && (cLink.indexOf('mailto')<0)) {
			           cLink = countryURL[getCountry()] + cLink;
			        }
			        elements[i].setAttribute('href',cLink.replace('https','http'));       
			    }
			}
		}
		else {
			setTimeout('HTTPlinks()',500);	
		}
	};
		...
		try {
			try{
				if('billingAddress' in orderObject && document.getElementById('billingAddress')) {
					...
					if(document.getElementById('email')) $('email').value = orderObject['billingAddress'].Email;
				}
			}
			catch(err){}
	...
	function processOrder() {
		try{
			...
			var storeID		= '';var webId;
			if(getCountry()=='US') {
				storeID	= 'USA';webId='WEB01';
			}
			else {
				storeID = 'CAN';webId='WEB02';	
			}
