	...
	<div id="toolbar" class="clearfix">
		<div id="breadcrumbs">
			<ul>
				...
				<#if (product.primary_genre?exists) && (product.primary_genre_name?exists)><li>&gt;<a href="/${product.primary_dept_name}/${product.primary_genre_name}">${product.primary_genre}</a></li></#if>
		...
		<div id="centercolumn">
			<div id="productdetail" class="clearfix">
				...
				<div class="right">
					...
					<#assign curModIndex = 0> <#if (product.role_1?exists)> <p><strong>Starring:</strong>
					<#list product.role_1 as role1> <#if (role1.name?exists)>
						<#if curModIndex = 0>  ${role1.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role1.name}</#if> 
					</#if> </#list></p> </#if> 
					<#assign curModIndex = 0> <#if (product.role_0?exists)> <p> <strong>Directed by:</strong>
					<#list product.role_0 as role0> <#if (role0.name?exists)>
						<#if curModIndex = 0>  ${role0.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role0.name}</#if> 
					</#if> </#list> </p></#if>
					<#assign curModIndex = 0> <#if (product.role_2?exists)> <p> <strong>Produced by:</strong>
					<#list product.role_2 as role2> <#if (role2.name?exists)>
						<#if curModIndex = 0>  ${role2.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role2.name}</#if> 
					</#if> </#list> </p></#if>
					<#assign curModIndex = 0> <#if (product.role_3?exists)><p>  <strong>Written by:</strong>
					<#list product.role_3 as role3> <#if (role3.name?exists)>
						<#if curModIndex = 0>  ${role3.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role3.name}</#if> 
					</#if> </#list></p> </#if>
					<#assign curModIndex = 0> <#if (product.role_4?exists)> <p> <strong>Narrated by:</strong>
					<#list product.role_4 as role4> <#if (role4.name?exists)>
						<#if curModIndex = 0>  ${role4.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role4.name}</#if> 
					</#if> </#list></p> </#if>
					<#assign curModIndex = 0> <#if (product.role_5?exists)><p>  <strong>Presented by:</strong>
					<#list product.role_5 as role5> <#if (role5.name?exists)>
						<#if curModIndex = 0>  ${role5.name}<#assign curModIndex = curModIndex+1> 
						<#else>, ${role5.name}</#if> 
					</#if> </#list> </p></#if>
